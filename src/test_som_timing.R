...
message("Small, 1 epoch, 2 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 1, mode = "pbatch", 
    init = som.init, cores = 2,
    normalizeDataLayers = FALSE)
b <- Sys.time()
print(b-a)

message("Small, 10 epochs, 2 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 10, mode = "pbatch", 
    init = som.init, cores = 2,
...
message("Small, 1 epoch, 4 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 1, mode = "pbatch", 
    init = som.init, cores = 4,
    normalizeDataLayers = FALSE)
b <- Sys.time()
print(b-a)

message("Small, 10 epochs, 4 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 10, mode = "pbatch", 
    init = som.init, cores = 4,
...
message("Large, 2 epochs, 4 cores")
a <- Sys.time()
som(train.1e6, grid = grid, 
    rlen = 2, mode = "pbatch", 
    init = som.init, cores = 4,
