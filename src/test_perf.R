...
if(model.id == "t18") {
  # Logit no intercept, Matèrn kappa 5/2
  # Model size
  k.def <- c(ten_loc.bl = 750,
             ten_loc.itpa = 500,
             ten_loc.ov = 250,
...
if(model.id == "t23") {
  # cauchit, normal intercept, large model
  # Model size
  k.def <- c(ten_loc.bl = 1000,
             ten_loc.itpa = 500,
             ten_loc.ov = 250,
...
if(model.id == "t24") {
  ...
  model <-
    bam(forestloss ~
        ...
        nthreads = n.threads,
        gc.level = 0,
        control = gam.control(trace = TRUE, epsilon = 5e-7)
        )
}
...
if(model.id == "t25") {
  ...
  model <-
    bam(forestloss ~
        ...
        nthreads = n.threads,
        gc.level = 0,
        control = gam.control(trace = TRUE, epsilon = 2e-7)
        )
}
