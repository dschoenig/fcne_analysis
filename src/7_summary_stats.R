...
list(
     ...
     stats.amz[adm0 %in% adm0.rec.amz & it_type == "recognized" & tmf_annual_2010 == 1,
               .(mar_type = "rec",
                 ...
                 area.rel = (.N*p.amz)/a.tmf.amz),
               by = .(it_type, pa_type, adm0)],
     stats.amz[adm0 %in% adm0.rec.amz & it_type == "recognized" & pa_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "recov",
                 area = .N*p.amz,
                 area.rel = (.N*p.amz)/a.tmf.amz)],
     stats.amz[adm0 %in% adm0.rec.amz & it_type == "recognized" & pa_type != "none" & tmf_annual_2010 == 1,
     ...
     stats.amz[adm0 %in% adm0.ind.amz & pa_type == "indirect_use" & tmf_annual_2010 == 1,
               .(mar_type = "ind",
                 ...
                 area.rel = (.N*p.amz)/a.tmf.amz),
               by = .(it_type, pa_type, adm0)],
     stats.amz[adm0 %in% adm0.ind.amz & pa_type == "indirect_use" & it_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "indov",
                 area = .N*p.amz,
                 area.rel = (.N*p.amz)/a.tmf.amz)],
     stats.amz[adm0 %in% adm0.ind.amz & pa_type == "indirect_use" & it_type != "none" & tmf_annual_2010 == 1,
...
list(
     ...
     stats.cam[adm0 %in% adm0.rec.cam & it_type == "recognized" & tmf_annual_2010 == 1,
               .(mar_type = "rec",
                 ...
                 area.rel = (.N*p.cam)/a.tmf.cam),
               by = .(it_type, pa_type, adm0)],
     stats.cam[adm0 %in% adm0.rec.cam & it_type == "recognized" & pa_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "recov",
                 area = .N*p.cam,
                 area.rel = (.N*p.cam)/a.tmf.cam)],
     stats.cam[adm0 %in% adm0.rec.cam & it_type == "recognized" & pa_type != "none" & tmf_annual_2010 == 1,
     ...
     stats.cam[adm0 %in% adm0.ind.cam & pa_type == "indirect_use" & tmf_annual_2010 == 1,
               .(mar_type = "ind",
                 ...
                 area.rel = (.N*p.cam)/a.tmf.cam),
               by = .(it_type, pa_type, adm0)],
     stats.cam[adm0 %in% adm0.ind.cam & pa_type == "indirect_use" & it_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "indov",
                 area = .N*p.cam,
                 area.rel = (.N*p.cam)/a.tmf.cam)],
     stats.cam[adm0 %in% adm0.ind.cam & pa_type == "indirect_use" & it_type != "none" & tmf_annual_2010 == 1,
...
list(
     ...
     stats.cam[adm0 %in% adm0.rec.cam.nh & 
               ...
               by = .(it_type, pa_type, adm0)],
     stats.cam[adm0 %in% adm0.rec.cam.nh & 
               hurr_lf == FALSE & it_type == "recognized" & pa_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "recov",
                 area = .N*p.cam,
                 area.rel = (.N*p.cam)/a.tmf.cam)],
     stats.cam[adm0 %in% adm0.rec.cam.nh & 
     ...
     stats.cam[adm0 %in% adm0.ind.cam.nh &
               ...
               by = .(it_type, pa_type, adm0)],
     stats.cam[adm0 %in% adm0.ind.cam.nh &
               hurr_lf == FALSE & pa_type == "indirect_use" & it_type != "none" & tmf_annual_2010 == 1,
               .(mar_type = "indov",
                 area = .N*p.cam,
                 area.rel = (.N*p.cam)/a.tmf.cam)],
     stats.cam[adm0 %in% adm0.ind.cam.nh &
