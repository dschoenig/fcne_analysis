...
## TENURE CATEGORIES AND ADMINISTRATIVE AREAS ################################## 
...
## TENURE CATEGORIES BY INDIVIDUAL AREAS #######################################
...
arc.ten_areas.plot <-
  ggplot(densities, aes(x = x, y = mean, ymin = q2.5, ymax = q97.5)) +
...
plots.combined <-
  ...
  (arc.ten_areas.plot
   + 
   theme(axis.title.y = element_text(hjust = 0, margin = margin(r = - 125, unit = "pt")),
...
arc.geo.all.map <- 
  ...
  scale_fill_continuous_divergingx(palette = "Roma",
                                  # ,n.breaks = 8,
                                  ,rev = TRUE,
                                  ,guide = guide_colorbar(ticks.colour = "grey5",
                                                          ticks.linewidth = 1,
                                                          frame.colour = "grey5",
                                                          frame.linewidth = 1,
                                                          barheight = 7.5,
                                   ...
                                   # ,labels = c("≤ −50", rep("", 4), 0, rep("", 4), "≥ +50"),
                                   # ,labels = c("≤ −50", "-40", "-30", "-20", "-10", 0,
                                               # "+10", "+20", "+30", "+40", "≥ +50"),
                                   # ,limits = c(-0.5, 0.5)
                                   ,breaks = seq(-0.25, 0.25, 0.05),
                                   # ,labels = c("≤ −25", rep("", 4), 0, rep("", 4), "≥ +25"),
                                   ,labels = c("≤ −25", "-20", "", "-10", "", 0,
                                               "", "+10", "", "+20", "≥ +25"),
                                   ,limits = c(-0.25, 0.25)
                                   ,oob = scales::squish
  ...
  #                                 ,oob = scales::squish
  #                                 ) +
  labs(fill = "Absolute change in\nrisk of forest loss [%]", x = NULL, y = NULL) +
  map_theme +
  theme(legend.position = "right",
        legend.justification = c(0,1),
...
quantile(arc.geo.all.sum$mean, c(0.005, 0.995))
with(arc.geo.all.sum, sum(mean > 0.25 | mean < -0.25) / length(mean))
