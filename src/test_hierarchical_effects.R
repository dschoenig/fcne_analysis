...
effects <- summarize_effects(models, amz.mod[1:1e4,], amz.data[1:1e4,])

effects_pan <- summarize_effects(models, amz.mod[amz.mod$adm0 == "PAN",], amz.data[adm0 == "PAN",])
effects_gtm <- summarize_effects(models, amz.mod[amz.mod$adm0 == "GTM",], amz.data[adm0 == "GTM",])
...
for(i in 1:length(models)) {
  predicted <- invlink_cll(summarise_draws(effects[[2]][[i]]$full, mean)$mean)
  response <- as.integer(amz.data$forestloss[1:1e4])
  mod.roc <- roc(response, predicted)
  plot(mod.roc)
  print(mod.roc)
}
...
for(i in 1:length(models)) {
  predicted <- invlink_cll(summarise_draws(effects_pan[[2]][[i]]$full, mean)$mean)
  response <- as.integer(amz.data[adm0 == "PAN", forestloss])
  mod.roc <- roc(response, predicted)
  plot(mod.roc)
  print(mod.roc)
}
