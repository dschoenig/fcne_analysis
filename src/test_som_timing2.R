...
message("Small, 1 epoch, 8 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 1, mode = "pbatch", 
    init = som.init, cores = 8,
    normalizeDataLayers = FALSE)
b <- Sys.time()
print(b-a)

message("Small, 10 epochs, 8 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 10, mode = "pbatch", 
    init = som.init, cores = 8,
...
message("Small, 1 epoch, 16 cores")
a <- Sys.time()
som(train.1e4, grid = grid, 
    rlen = 1, mode = "pbatch", 
...
message("Large, 1 epoch, 16 cores")
a <- Sys.time()
som(train.1e6, grid = grid, 
    rlen = 1, mode = "pbatch", 
    init = som.init, cores = 16,
...
message("Large, 2 epochs, 16 cores")
a <- Sys.time()
som(train.1e6, grid = grid, 
    rlen = 2, mode = "pbatch", 
    init = som.init, cores = 16,
